...
mod tests {
    ...
    use crate::store::tests::Event::{
        FirstValueUpdate, OtherUpdate, SecondValueUpdate, ThirdValueUpdate,
    };
    ...
    impl Reducer<SimpleState> for SimpleReducer {
        ...
        fn reduce(&self, state: &SimpleState, event: Self::Event) -> SimpleState {
            match event {
                FirstValueUpdate { value } => SimpleState {
                    first_value: value,
                    ..*state
                },
                SecondValueUpdate { value } => SimpleState {
                    second_value: value,
                    ..*state
                },
                ThirdValueUpdate { value } => SimpleState {
                    third_value: value,
                    ..*state
                },
                _ => state.clone(),
