...
where
    F: Fn(&mut S, &E) -> ReducerOutput<E>,
{
    type Event = E;
    type Slice = S;

    fn reduce(&self, slice: &mut Self::Slice, event: &Self::Event) -> ReducerOutput<E> {
    ...
    ($state:ident, $($field:ident: $slice:ty),+ $(,)?) => {
        ...
        $(
            impl HasSlice<$slice> for $state {
                fn slice(&mut self) -> &mut $slice { &mut self.$field }
            }
        )+
    };
}
...
pub(crate) mod shared_tests {
    ...
    pub(crate) fn updates_first_slice<R: Reducer<ExampleState, Event = Event>>(root_reducer: R) {
        ...
        let _ = root_reducer.reduce(&mut state, &FirstValueUpdate { value: 2 });
        assert_eq!(
            state,
            ExampleState {
    ...
    pub(crate) fn updates_second_slice<R: Reducer<ExampleState, Event = Event>>(root_reducer: R) {
        let mut state = make_state();
        let original = state.clone();
        let _ = root_reducer.reduce(&mut state, &SecondValueUpdate { value: 1.7 });
        assert_eq!(
            state,
            ExampleState {
